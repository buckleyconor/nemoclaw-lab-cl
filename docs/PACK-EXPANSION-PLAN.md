...
- **One docker-compose stack serves exactly one pack.** `PACK_ID` (`docker-compose.yaml:15`) is a single global env var read independently at startup by all four services (`gateway`, `mcp-tools`, `orchestrator`, `simulator`). There is no per-request or per-session pack selection — switching packs means restarting the stack with a different `PACK_ID`, via `make switch-pack PACK_ID=<id>` (`deploy/scripts/switch-pack.sh`), which updates `.env` and fully recreates the stack (a plain `up -d` isn't enough — `mcp-tools`' FAISS index and the gateway's in-memory fault/activity registry are pack-scoped and must not survive a switch).
  Because the switch is a manual, non-atomic host action, `docs/lab-guide.html`'s vertical picker can end up disagreeing with what the stack is actually serving (e.g. the operator picks a vertical but forgets to run `switch-pack` first). The guide detects this itself: after the dashboard iframe loads, it fetches `GET /api/pack` and compares `pack.id` against the picked vertical's expected pack id, showing a banner with the exact `switch-pack` command to run if they don't match.
- **One OpenClaw sandbox serves exactly one pack**, for the same reason plus one more: `openclaw/SOUL.md`'s Identity paragraph hardcodes *"an autonomous AI agent deployed on a DELL PowerEdge XE9780L GPU compute cluster equipped with NVIDIA B300 GPUs."* This was flagged as a known gap in ADR-010's consequences and never resolved. `openclaw/skills/infra-sentinel-monitor/SKILL.md` has one similarly hardcoded phrase ("on the GPU cluster"). Everything else under `openclaw/` — `AGENTS.md`, the other three skill files, and all of the `nemoclaw-infra-tools` plugin's TypeScript — is already pack-agnostic and needs no changes per pack.
  In practice this box runs a single shared sandbox reused across packs (not one-per-pack), so its SOUL.md/AGENTS.md/skills carry whichever pack's persona content was pasted in last. `docs/lab-guide.html`'s "Ready for a different scenario?" button blanks all six before returning to the vertical picker — a `POST /api/terminal/reset` call, proxied by the Gateway (`services/gateway/terminal.py`) to the terminal daemon's `POST /reset` (`services/terminal/main.py`, restricted mode only), which is the non-interactive equivalent of the restricted console's reset action (menu option `r`, `services/terminal/console.py::run_reset()`).
