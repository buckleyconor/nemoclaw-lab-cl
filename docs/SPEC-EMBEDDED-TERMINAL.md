...
- **Endpoint `WS /ws`, `restricted` mode (ADR-013, M9):** spawns `python -m services.terminal.console` instead of a shell. Requires `SANDBOX_NAME` (the tenant's sandbox — fails fast at `create_app()` time, like `TERMINAL_TOKEN`). The console presents a fixed 6-item menu (`SOUL.md`, `AGENTS.md`, 4 skills) plus a reset action (letter key `r`, never numbered so it can't be mistaken for a seventh setup step); every sandbox path is hardcoded, every subprocess call uses a literal argv list. See `services/terminal/console.py` and ADR-013 for the full design. One daemon process per tenant (`deploy/scripts/run-terminal-tenant.sh`), never shared across tenants.
- **Endpoint `POST /reset`, `restricted` mode only:** non-interactive equivalent of the console's reset action (menu option `r`, `console.run_reset()`) — blanks and re-pushes SOUL.md/AGENTS.md/all four skills without a live PTY session, so a plain authenticated POST can trigger it. Requires the same `Authorization: Bearer <token>` as `/ws`; not registered at all in `shell` mode (404), since reset is a sandbox-persona concept with no meaning for a raw login shell. Runs `run_reset()` in a threadpool executor — it does blocking `subprocess` calls and must not stall the daemon's concurrent PTY pumps. This is what backs `docs/lab-guide.html`'s "Ready for a different scenario?" button.
...
- `POST /api/terminal/reset` → best-effort forward to the daemon's `POST /reset` over plain HTTP (not WS — it's a single request/response), reusing the same enabled/URL/token check as `/api/terminal/enabled` and the `httpx.AsyncClient` already used elsewhere in the Gateway. Returns `{"ok": false, "error": ...}` rather than raising when the terminal is disabled or the daemon is unreachable, since the caller (`lab-guide.html`) must never block navigation on this.
